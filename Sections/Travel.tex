...
% \begin{itemize}
%             \item For hybrid or virtual events facilitate engagement and interaction by using the appropriate software, equipment and infrastructure.            
%             \item For regular in-person conferences, consider making every \textit{n}-th occurrence virtual.
%             \item Merge conferences with overlap in attendees and consider travel emissions when selecting conference location.
% \end{itemize}
\end{itemize}
}
{
...
\end{itemize}
% Where travel is necessary, incentivise and enable travel by more environmentally sustainable means, despite potentially higher costs. Advocate for better regional sustainable transport infrastructure.
% \begin{itemize}
%             \item Provide bicycle-friendly infrastructure, such as covered, secure bike parking and on-site showers.
%             \item Subsidise costs of public transport for work commutes and disincentivise the use of private cars for commuting where viable alternatives exist.
%             \item Where public transport is sparse, provide shuttle services for daily commutes, encourage and facilitate car pooling, and provide on-site charging stations for electric vehicles.
%             \item Push campus travel agents to include multi-modal travel options in their route searches, especially for international travel.
%             \item Refrain from using extent of travel as an indication of quality in hiring decisions.
%             \item Implement carbon budgets for discretionary business travel (with exceptions for early-career researchers), using carbon offsetting only as a last resort.
%         \end{itemize}
}
\end{reco2}
...
% \caption[Emissions % factors 
% for commuting]{GHG emissions %factors 
% for different means of transport (in g\CdOe\ per km). 
% Emissions from electricity and vehicle production as well as fuel combustion are included.  All data is for 2019-2020, and comes from the database of Ref.~\cite{labos1p5} --- see, in particular, Ref.~\cite{labos1p5emi} --- and assumes the electricity comes from the French grid, which is a factor of 10 less carbon-intensive than other countries~\cite{OWDintensity}.
% Public-transit figures are per-person.
% \label{tab:emiCommute}}
% % Sorted by Alphabet

% \begin{tabular}{@{}m{0.1\textwidth}m{0.4\textwidth}m{0.15\textwidth}m{0.17\textwidth}@{}}
%    \toprule
%     &Vehicle&
%     emissions (g \CdOe/km) & Fractional uncertainty ($\pm$)\\
%     \midrule
%     \parbox[t]{2mm}{\multirow{8}{*}{\rotatebox[origin=c]{90}{Personal transit}}}&bicycle & 5.0 & 0.7 \\
%     &electric bicycle & 10.9 & 0.5\\
%     &electric scooter &  24.9 & 0.5 \\
%     &electric car & 103.4 & 0.7 \\
%     &hybrid car &  182.8 & 0.7\\
%     &motorbike &  202.8 & 0.6\\
%     &diesel-powered car &  209.6 & 0.6 \\
%     &gasoline-powered car &  240.1 & 0.6\\
%     \midrule
%     \parbox[t]{2mm}{\multirow{8}{*}{\rotatebox[origin=c]{90}{Mass transit}}}&subway &  3.5 & 0.2 \\
%     &tramway > 250,000 inhabitants &  4.0 & 0.6\\
%     &commuter rail (RER/transilien) &  5.1 & 0.2 \\
%     &tramway =< 250,000 inhabitants &  5.7 & 0.6\\
%     &rail (< 200 km) &  18.0 & 0.6 \\
%     &bus >250,000 inhabitants &  135 & 0.6\\
%     &bus 100,000--250,000 inhabitants  & 146 & 0.6\\
%     &bus <100,000 inhabitants &  156 & 0.6\\
%    \bottomrule
% \end{tabular}
% \end{table}
% %%%%%
...
\begin{casestudy}[Travel to CERN\label{case:traveltoCERN}]{Travel to CERN}%
...
\begin{casestudy}[Comparative study of travel carbon footprint for ICHEP conferences (2012--2020)\label{case:ICHEP}]{Comparative study of travel carbon footprint for ICHEP conferences (2012--2020)}% 
...
\begin{bestpractice}[Internal regulations to reduce the impact of business travel at DESY\label{bp:DESY_travel}]{Internal regulations to reduce the impact of business travel at DESY}%
...
%As it was intended from its conception to be an "online first" conference, it is able . Thus, the format is a useful supplement to the traditional, in-person, conference format. 

%There are many benefits that an online conference can bring. Indeed,
% CfH has run on a yearly basis since 2020, and has consistently been described as pleasant, productive, and useful. The goal of this case study is therefore to illustrate what can be accomplished when the conference format is reconsidered from the perspective on the internet. Additionally, it is hoped that this format can serve as an inspiration for others to improve on the way conferences are structured.

%\noindent {\bfseries Format}
...
%By having the participants meet in the virtual conference venue, the participants can wander around the conference, popping in and out of conversations. The creates a pleasant and social atmosphere, which then leads to follow-up discussions and, potentially, further collaboration after the conference.

% The aspects of CfH that distinguish it from other conferences can be classified in three broad groups: presentations, discussions and interactions. These can again be split into scientific and social groups. Each of these is explained in further detail below.

% \noindent {\bfseries Scientific presentations}

% CfH is entirely built around the advantages that online technologies can give to scientific conferences. One important decision was to largely separate scientific presentations and discussions. Since presentations do not involve much direct speaker-to-audience interaction, talks are recorded and submitted before the start of the conference. CfH places few constraints on the format of the talks. The main restriction is that the speakers stay within the time limit. The recordings of the talks are then shared with the conference participants ahead of the start of the conference. The participants can view the talks at their leisure before, during and after the conference.

% \noindent {\bfseries Scientific Discussions:\ Asynchronous Communication}
...
%\footnote{It should, however, be noted that Discord’s threading function is less developed than the other examples listed here. Similarly, Discord does not currently support archiving of data.})
...
%The exact type of message board is not important; what is important is that every conference participant is easily able to access announcements and discussion topics.

% \noindent {\bfseries Scientific Discussions:\ Live Communication}

% CfH features both scheduled and spontaneous “informal” live discussions. The scheduled talk discussions are the closest analogue to the plenary and parallel sessions of traditional conferences: the main topic of discussion are the pre-recorded talks. 

% Since all participants in the discussion are expected to have watched the talks, every speaker need only give a very brief, one-slide summary of their talk.
% %A large part of the scheduled discussion time can be dedicated discussing the talks and gathering ideas for discussion later during the conference.
% Previous asynchronous discussion can be used to expand the scope of the original talk discussion, and the participants are encouraged to continue the discussions in the various rooms of the virtual conference venue.
...
%Moreover, in contrast to the scheduled live discussions, there is no limit on the duration or number of separate informal discussions. 
...
% Social games and casual get-togethers in a welcoming atmosphere will happen at times that reflect the organic end of the informal discussions on the day. 
These activities aim to reproduce (at least partially) the evening interactions of in-person conference social events.

%\noindent {\bfseries Benefits:\ Benefits of an online conference venue}
...
%as the main requirement for participation in the conference is a stable internet connection.
%As this format becomes more well known and established within the research community, this will hopefully result in increasingly diverse backgrounds of the conference participants.\footnote{As the format is still young, it is too early to tell whether our present participant makeup is representative.}
...
%%%%%Because the entire conference is online it is straightforward to archive scientific discourse. The talks are permanently\footnote{Permanent for as long as YouTube exists, that is.} available at the CfH’s YouTube channel~\cite{CfHchannel} for the conference participants. The discussions can also be easily stored and made accessible for the participants. Furthermore, speakers can choose to make their talk accessible to people outside of the conference; these talks can be found on CfH’s YouTube channel~\cite{CfHchannel}. 

%\noindent {\bfseries Benefits:\ Benefits of pre-recorded talks}
...
% Additional advantages of this approach are the ability to pause the talk, restart the talk, catch up on background details and take more time to think of questions.

% Because the precise format of the recordings is unrestricted, the speakers are free to experiment with novel ways to present their research. Many have done so: over the years researchers have submitted talks that are accessible, engaging and memorable, as can be seen on the YouTube channel. Because all speakers have their talks recorded, this helps spread the results of their research.

% \noindent {\bfseries Benefits:\ Benefits of asynchronous discussion}

 % Discussion before the live-session of the talk can be used to clear up confusion or highlight where there is disagreement, and this can be partially alleviated before the live discussion. The asynchronous discussion can also be used to enrich the live talk discussion.

% The asynchronous discussions can take place before, during and after the scheduled talk discussions outlined above. Because of this, talks can be discussed throughout the conference, which increases the exposure of researchers.

% \noindent {\bfseries Benefits:\ Benefits of the informal live discussions}

% \noindent {\bfseries Interaction between the conference participants}

% This is an essential part of the conference format, as this format lends itself the most to traditional mingling and networking.

%\noindent {\bfseries Community reception}
...
% because it was the first year that the format was run. In subsequent years we found increasingly active and enthusiastic participation in all of the above components:
% Speaker participants created innovative and accessible talk recordings.
% Participants organised watch parties, where they gathered on Sococo to watch and discuss the talks.
% Participants organised impromptu discussions, on scientific, technical and social topics.
% The themed discussions proved to be very popular; most of the conference iterations so far have had to arrange for parallel sessions during the themed discussion slots because the number of topics suggested was very high. These sessions are always well attended.
% Participants used various breakout rooms to continue discussion after the scheduled talk discussions were finished.
% Text-based discussions were active for the whole duration of the conference.
% Participants regularly referred to the recorded discussions, both text-based and live, after the conference.

% It should be noted that the organisers actively encouraged the participants to partake of the various aspects of the conference, and actively set examples.
...
% Although there is a decrease from 2021 to 2022, this can be attributed to the COVID-19 pandemic. Few in-person conferences were organised in the years 2020–2021.

% Once in-person conferences picked up again in 2022, it is expected that the number of participants of CfH receded to a likely equilibrium. We are therefore optimistic that this format is viable in the long term in HECAP. 
...
%     \centering
%     \captionsetup{type=figure}
%     \includegraphics[width=1.\textwidth]{Sections/Figs/Travel/participants.png}
%     \caption[Number of active CfH participants for each of the installments by year.]%
%         {Number of active CfH participants for each of the installments by year.\label{fig:CfH2}}
% \end{center}

%%%%%

% \noindent {\bfseries Conclusion}

% Based on the past 3 years of experience, it can be said with confidence that CfH is a unique and successful example of what can be accomplished if one considers a conference as “online first”. Material generated during the conference can be easily stored and referred to in the future, providing a lasting record of discussion and collaboration. We encourage people to experiment with the format, for large-scale conferences or small-scale workshops.
